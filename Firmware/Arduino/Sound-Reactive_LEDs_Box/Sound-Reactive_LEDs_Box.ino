...
#define LED_TOTAL 56  //Change this to the number of LEDs in your strand.
...
#define BUTTON_0  3   //Capacitive Touch Button 0 toggles visulaization mode, color palette, and brightness

boolean prev_BUTTON_0State = false;
boolean current_BUTTON_0State = false;
int vcb_mode = 0;
...
void loop() {  //This is where the magic happens. This loop produces each frame of the visual.
  ...
  */
  BUTTON_0State = digitalRead(BUTTON_0);
  ...
  //knob = 550 / 1023.0; //adjusted later in code...vcb_mode...
  ...
  if (BUTTON_0State == LOW) {
    current_BUTTON_0State = true; // button has been pressed once

    if (prev_BUTTON_0State != current_BUTTON_0State) { //check to see if button is still being pressed
      vcb_mode = vcb_mode + 1; //change visual, color, and brightness MODE after button has been pressed
      if (vcb_mode < 0 || vcb_mode > 3) {
        //reset vcb_mode
        vcb_mode = 0;
      }
      ...
      switch (vcb_mode) {
        case 1:
          //visual = 4; //PaletteDance
          visual = 3; //Snake
          palette = 0;
          knob = 50 / 1023.0;
          
          for (int i = 0; i < 15; i++) {
            strand.setPixelColor(i, strand.Color(255, 0, 0));
            strand.show();
            delay(5);
          }
        ...
        case 2:
          visual = 2; //Traffic
          palette = 3;//Pinacolada
          knob =  50 / 1023.0;
          //palette = 6;//ROY
          //palette = 4;//Sulfur

          for (int i = 0; i < 15; i++) {
            strand.setPixelColor(i, strand.Color(0, 255, 0));
            strand.show();
            delay(5);
          }
        ...
        case 3:
          visual = 6; //Paintball
          palette = 2;//Ocean, blue-green
          knob = 50 / 1023.0;

          for (int i = 0; i < 15; i++) {
            strand.setPixelColor(i, strand.Color(0, 255, 255));
            strand.show();
            delay(5);
          }
          knob = 300 / 1023.0;
        ...
        default:
          ...
          for (int i = 0; i < 15; i++) {
            strand.setPixelColor(i, strand.Color(255, 255, 0));
            strand.show();
            delay(5);
          }
    ...
    prev_BUTTON_0State = current_BUTTON_0State;
  }
  //button has not been pressed, it will be high
  else {
    current_BUTTON_0State = false;
    prev_BUTTON_0State = current_BUTTON_0State;
  }
...
uint32_t ColorPalette(float num) {
  switch (palette) {
    ...
    //case 6: return (num < 0) ? ROY(gradient) : ROY(num);
...
uint32_t Rainbow(unsigned int i) {
  ...
  if (i > 1274) return strand.Color(255, 0, 255 - (i % 255));   //violet(magenta) -> red
  if (i > 1019) return strand.Color((i % 255), 0, 255);         //blue -> violet(magenta)
  if (i > 764) return strand.Color(0, 255 - (i % 255), 255);    //aqua(CYAN) -> blue
  if (i > 509) return strand.Color(0, 255, (i % 255));          //green -> aqua(CYAN)
...
uint32_t ROY(unsigned int i) {//not working...
  if (i > 764) return ROY(i % 765);
  if (i > 509) return strand.Color(255, 128 - (i % 255) / 2, 0);        //orange(255,128,0) -> red(255,0,0)
  if (i > 255) return strand.Color(255, 255 - (i % 255) / 2, 0);        //yellow(255,255,0) -> orange(255,128,0)
  return strand.Color(255, i, 0);                                     //red(255,0,0) -> yellow(255,255,0)
}
